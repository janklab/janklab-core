classdef TestTwoFields
    ...
    methods
        function [keysA,keysB] = proxyKeysOLD(a, b)
        keysA = [a.foo(:) a.bar(:)];
        if nargin > 1
            keysB = [b.foo(:) b.bar(:)];
        end
    ...
    methods
        ...
        if nargin > 1
            ...
            [this.bar,nshifts] = shiftdim(this.bar);
        end
        end
        ...
        args = varargin;
        for i = 1:numel(args)
            if ~isa(args{i}, 'jl.code.TestTwoFields')
                args{i} = jl.code.TestTwoFields(args{i});
            end
        end
        out = args{1};
        ...
        function out = eq(a, b)
        %EQ == Equal.
        if ~isa(a, 'jl.code.TestTwoFields')
            a = jl.code.TestTwoFields(a);
        end
        if ~isa(b, 'jl.code.TestTwoFields')
            b = jl.code.TestTwoFields(b);
        end
        tf = a.foo == b.foo;
        ...
        if ismember('rows', varargin)
            ...
            if isa(keys, 'table')
                [keys,ix] = unique(keys, flags{:});
            else
                [keys,ix] = unique(keys, 'rows', flags{:});
            end
            out = [subset(nonnans(ix)); nans];
            Indx = [ixNonnan(ix); find(tfNaN)];
            out = subset(this, Indx);
    ...
    methods (Access=private)
        ...
        if ~isa(rhs, 'jl.code.TestTwoFields')
            rhs = jl.code.TestTwoFields(rhs);
        end
