classdef PlanarClassGenerator
  ...
  properties
    ...
    boilerplateLocalFcnHeader string = '%%%%% START PLANAR-CLASS BOILERPLATE LOCAL FUNCTIONS %%%%%';
    boilerplateLocalFcnFooter string = '%%%%% END PLANAR-CLASS BOILERPLATE LOCAL FUNCTIONS %%%%%';
    boilerplateMessage string = strjoin({
    ...
    specialDelegates string = {
      'shiftdim'
      'cat'
      'horzcat'
      'vertcat'
      };
    special string = {'subsasgn' 'subsref'};
  end
  ...
  methods
    ...
    function validateDefinition(this, defn) %#ok<INUSL>
      ...
      if ~ismissing(defn.planarNanFlag) && ~ismember(defn.planarNanFlag, defn.planarFields)
    ...
    function genBoilerplate(this, file)
      ...
      try
        ...
        try
          ...
          fprintf(['ERROR: Unable to restore original state of file ''%s''.\n' ...
            'Error: %s\n' ...
    ...
    function out = genBoilerplateForBaseClass(this, fqClassName, notes)
      ...
      publicDefns = string([]);
      protectedDefns = string([]);
      privateDefns = string([]);
      localFcnDefns = string([]);
      ...
      out = jl.code.internal.PlanarClassBoilerplateParts;
    ...
    function out = genBoilerplateForStandalone(this, fqClassName, notes)
      ...
      function out = subst(str)
        ...
        out = str;
        for iVar = 1:numel(varnames)
          if ~isempty(vars.(varnames{iVar})) && ~any(ismissing(vars.(varnames{iVar})))
            out = strrep(out, ['<' varnames{iVar} '>'], vars.(varnames{iVar}));
          end
        end
      end
      ...
      if ~notes.hasUserIsNan
        ...
        defn{end+1} = 'end';
        substDefn = subst(defn);
        defn = strjoin(substDefn, '\n');
      ...
      if ismember('proxyKeys', notes.userMethodNames)
        ...
        defn = subst(strjoin({
          ...
          ['propertyValsA = {' strjoin(strcat('a.', cellstr(planarFields)), ' ') '};']
          ...
          ['        propertyNames = {' strjoin(strcat('''', cellstr(planarFields), ''''), ' ') '};']
          '        keysA = table(propertyValsA{:}, ''VariableNames'', propertyNames);'
          '    end'
          'else'
          ['    propertyValsB = {' strjoin(strcat('b.', cellstr(planarFields)), ' ') '};']
